...
SECTION_SPECS: tuple[SectionSpec, ...] = (
    ...
    SectionSpec(
        ...
        fields=(
            ...
            FieldSpec("backend", "permission_mode", "Permission mode", "choice", _CHOICE_PERMISSION_MODES, help_text="How much the coding agent may DO. plan: read-only, cannot write or run commands (safe default). approve: may act, but every write/command needs voice approval via your approval_phrase -- real on both Codex (native per-call approval channel) and Claude Code (a PreToolUse hook this adapter builds itself, since headless mode has no native one -- see claude_code.py's module docstring). While a request is pending, say 'explain'/'explanation'/'clarify'/'help' to have the full detail read back before deciding, or 'no' to deny -- the prompt stays open across a clarifying exchange. permissive: acts without asking (dangerous). No effect on opencode (set at `opencode serve` launch). Do NOT also set a permission flag in Command -- that's a conflict."),
