...
from convobox.resumeword import DEFAULT_RESUME_WORD, ResumeWordDetector


@pytest.fixture
def detector() -> ResumeWordDetector:
    return ResumeWordDetector(resume_word="Athena")


def test_exact_match(detector: ResumeWordDetector) -> None:
    assert detector.check("Athena") is True


def test_case_insensitive(detector: ResumeWordDetector) -> None:
    assert detector.check("ATHENA") is True


def test_embedded_in_sentence(detector: ResumeWordDetector) -> None:
    assert detector.check("hey Athena, hold on a second") is True


def test_tolerates_punctuation_and_extra_whitespace(detector: ResumeWordDetector) -> None:
    assert detector.check("...Athena!") is True


def test_no_false_positive_on_substring(detector: ResumeWordDetector) -> None:
    # word-boundary aware: not a match inside a larger word
    assert detector.check("Athenaeum is a nice word") is False


def test_no_match_on_unrelated_speech(detector: ResumeWordDetector) -> None:
    assert detector.check("can you run the tests again") is False


def test_empty_transcript(detector: ResumeWordDetector) -> None:
    assert detector.check("") is False


def test_whitespace_only_transcript(detector: ResumeWordDetector) -> None:
    assert detector.check("   \t\n  ") is False


def test_multi_word_wake_phrase() -> None:
    detector = ResumeWordDetector(resume_word="hey Athena")
...
def test_resume_word_property_returns_original() -> None:
    detector = ResumeWordDetector(resume_word="Hey Athena")
    assert detector.resume_word == "Hey Athena"
...
def test_phrase_normalizing_to_empty_raises() -> None:
    with pytest.raises(ValueError, match="normalizes to nothing"):
        ResumeWordDetector(resume_word="!!!")
...
def test_common_words_are_allowed_as_resume_words(casual: str) -> None:
    # A resume word is the push-word barge-in trigger, not an approval gate --
    ...
    detector = ResumeWordDetector(resume_word=casual)
