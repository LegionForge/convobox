...
| **Linux** | Mostly validated | AEC (N=10 volume sweep, twice), Kokoro TTS, and TUI/text/Web UI (real browser-driven testing) are all confirmed live. All three backends have now had a real mic-loop pass: claude-code got a full real-human-voice session (safeword hard-stop, `kill_phrase`, self-barge-in all confirmed); Codex got a real spoken question → real tool call → correct spoken answer, plus a clean `kill_phrase` force-kill; OpenCode's orchestrator-wired real-mic path was exercised for the first time and surfaced a real infinite-hang bug (a provider-rejected model's error event never clearing busy state), fixed the same session — a clean successful turn through that exact path is still unconfirmed. A live acoustic safety-phrase sweep also found some phrases (e.g. "abort abort abort", and Kokoro's own rendering of "eject eject eject") less reliable via synthesized voices than assumed — see docs/field-notes/. Stays here pending that OpenCode confirmation and sustained everyday use. |
